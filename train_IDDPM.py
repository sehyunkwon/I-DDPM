...
from diffusion import Gaussian_Improved_DiffusionTrainer, GaussianDiffusionSampler
...
flags.DEFINE_string('fid_cache', './stats/cifar10.npz', help='FID cache')
...
def train():
    ...
    trainer = Gaussian_Improved_DiffusionTrainer(
        net_model, FLAGS.beta_1, FLAGS.beta_T, FLAGS.T, FLAGS.k).to(device)
...
def eval():
    ...
    save_image(
        ...
        os.path.join(FLAGS.logdir, 'samples.pdf'),
        nrow=16)
    ...
    save_image(
        torch.tensor(samples[:256]),
        os.path.join(FLAGS.logdir, 'samples_ema.pdf'),
        nrow=16)
